...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(base_name, "aetherzenith") == 0) {
        if (argc < 2) {
            fprintf(stderr, "Usage: azenith <1|2|3>\n");
        ...
        FILE *fp = fopen("/sdcard/AZenith/config/value/AIenabled", "r");
        if (fp) {
            if (fgets(ai_state, sizeof(ai_state), fp)) {
    ...
    systemv("rm -f /sdcard/AZenith/debug/AZenith.log");
    systemv("rm -f /sdcard/AZenith/debug/AZenithVerbose.log");
    systemv("rm -f /sdcard/AZenith/preload/AZenithPR.log");
    ...
    FILE *fp = fopen("/sdcard/AZenith/config/value/AIenabled", "r");
    if (fp) {
        if (fgets(prev_ai_state, sizeof(prev_ai_state), fp)) {
            prev_ai_state[strcspn(prev_ai_state, "\r\n")] = 0;
        }
        fclose(fp);
    } 
    ...
    while (1) {
        ...
        FILE *fp = fopen("/sdcard/AZenith/config/value/AIenabled", "r");
        if (fp) {
            if (fgets(ai_state, sizeof(freqoffset), fp)) {
                ai_state[strcspn(ai_state, "\r\n")] = 0;
            }
            fclose(fp);
        } 
